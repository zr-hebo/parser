...
func (s *testParserSuite) TestPrivilege(c *C) {
	table := []testCase{
		...
		{`CREATE USER IF NOT EXISTS 'rdsroot'@'%' IDENTIFIED WITH 'mysql_native_password' AS 'hashstring' REQUIRE NONE PASSWORD EXPIRE DEFAULT ACCOUNT UNLOCK PASSWORD HISTORY DEFAULT PASSWORD REUSE INTERVAL DEFAULT PASSWORD REQUIRE CURRENT DEFAULT`, true, "CREATE USER IF NOT EXISTS `rdsroot`@`%` IDENTIFIED WITH 'mysql_native_password' AS 'hashstring' REQUIRE NONE PASSWORD EXPIRE DEFAULT ACCOUNT UNLOCK PASSWORD HISTORY DEFAULT PASSWORD REUSE INTERVAL DEFAULT PASSWORD REQUIRE CURRENT DEFAULT"},
